    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, args.max_epoch):
            ...
            for phase in ['source_train', 'source_val', 'target_val']:
                ...
                for batch_idx, (inputs, labels) in enumerate(self.dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'source_train'):
                        ...
                        if phase == 'source_train' and epoch >= args.middle_epoch:
                            ...
                            domain_label_source = torch.ones(labels.size(0), 1).float()
                            domain_label_target = torch.zeros(inputs.size(0) - labels.size(0), 1).float()
                ...
                # tb_writer.add_scalar('source_train',epoch_loss,epoch)
                # tb_writer.add_scalar('source_train', epoch_acc, epoch)
                # tb_writer.add_scalar('source_val', epoch_loss, epoch)
                # tb_writer.add_scalar('source_val', epoch_acc, epoch)
                # tb_writer.add_scalar('target_val', epoch_loss, epoch)
                # tb_writer.add_scalar('target_val', epoch_acc, epoch)
                # save the model
                if phase == 'target_val':

                    # acc.append(round(epoch_acc,2))
                ...
                if phase == 'target_val' and epoch == args.max_epoch - 1:
                    labels, pred = labels.cpu().detach().numpy(), pred.cpu().detach().numpy()
                    f1 = f1_score(labels, pred, average='micro')
                    labels, pred = torch.Tensor(labels).to(self.device), torch.Tensor(pred).to(self.device)
                    f1_list.append(f1)
